...
class hotel_restaurant_reservation(osv.osv):
    ...
    def onchange_partner_id(self, cr, uid, ids, part):
        ...
        addr = self.pool.get('res.partner').address_get(
            cr, uid, [part], ['default'])
        self.pool.get('res.partner').browse(
            cr, uid, part).property_product_pricelist_purchase.id
    ...
    def table_reserved(self, cr, uid, ids, *args):
        ...
        for reservation in self.browse(cr, uid, ids):
            ...
            cr.execute("select count(*) from hotel_restaurant_reservation as hrr "
                       ...
                       "inner join reservation_table as rt on rt.reservation_table_id = hrr.id "
                       "where hrr.id= %s) ", (reservation.start_date, reservation.end_date, reservation.id, reservation.id))
            ...
            if roomcount:
                raise osv.except_osv(
                    'Warning', 'You tried to confirm reservation with table those already reserved in this reservation period')
            else:
    ...
    def generate_kot(self, cr, uid, ids, part):
        ...
        for order in self.browse(cr, uid, ids):
            ...
            for order_line in order.order_list:
                ...
                self.pool.get('hotel.restaurant.order.list').create(
                    cr, uid, o_line)
