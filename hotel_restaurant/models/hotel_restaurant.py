...
class HotelRestaurantReservation(models.Model):
    ...
    start_date = fields.Datetime(
        string="Start Time", required=True, default=lambda self: fields.Datetime.now()
    )
...
class HotelRestaurantKitchenOrderTickets(models.Model):
    ...
    kot_date = fields.Datetime(string="Date")
...
class HotelRestaurantOrder(models.Model):
    ...
    order_no = fields.Char(string="Order Number", readonly=True)
    o_date = fields.Datetime(
        string="Order Date", required=True, default=lambda self: fields.Datetime.now()
    )
    room_id = fields.Many2one("product.product", string="Room No")
    folio_id = fields.Many2one("hotel.folio", string="Folio No")
    waiter_id = fields.Many2one("res.partner", string="Waiter")
    table_nos_ids = fields.Many2many(
        "hotel.restaurant.tables",
    ...
    is_folio = fields.Boolean(
        string="Is a Hotel Guest??", help="is customer reside in hotel or not"
    )
    customer_id = fields.Many2one("res.partner", string="Customer Name", required=True)
...
class HotelReservationOrder(models.Model):
    ...
    def reservation_generate_kot(self):
        ...
        for order in self:
            ...
            line_data = {
                ...
                "waiter_name": order.waiter_name.name,
                "table_nos_ids": [(6, 0, table_ids)],
            }
            kot_data = order_tickets_obj.create(line_data)
            for order_line in order.order_list_ids:
    ...
    order_number = fields.Char(string="Order No", readonly=True)
    reservation_id = fields.Many2one(
        "hotel.restaurant.reservation", string="Reservation No"
    )
    ...
    order_list_ids = fields.One2many(
        "hotel.restaurant.order.list", "reservation_order_id", string="Order List"
    )
    tax = fields.Float(string="Tax (%) ")
    amount_subtotal = fields.Float(
        compute="_compute_amount_all_total", string="Subtotal"
    ...
    amount_total = fields.Float(compute="_compute_amount_all_total", string="Total")
    kitchen = fields.Integer(string="Kitchen Id")
...
class HotelRestaurantOrderList(models.Model):
    ...
    restaurant_order_id = fields.Many2one(
        "hotel.restaurant.order", string="Restaurant Order"
    )
    reservation_order_id = fields.Many2one(
        "hotel.reservation.order", string="Reservation Order"
    )
    kot_order_id = fields.Many2one(
        "hotel.restaurant.kitchen.order.tickets", "Kitchen Order Tickets"
    )
    menucard_id = fields.Many2one("hotel.menucard", string="Item Name", required=True)
    item_qty = fields.Integer(string="Qty", required=True, default=1)
    item_rate = fields.Float(string="Rate")
