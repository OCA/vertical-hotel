...
class HotelReservation(models.Model):
    ...
    partner_id = fields.Many2one("res.partner", "Guest Name", readonly=True, index=True,
        required=True, states={"draft": [("readonly", False)]}, )
    pricelist_id = fields.Many2one("product.pricelist", "Scheme", required=True,
    ...
    partner_order_id = fields.Many2one("res.partner", "Ordering Contact", readonly=True,
        states={"draft": [("readonly", False)]}, help="The name and address of the "
                                                      "contact that requested the order "
                                                      "or quotation.")
    partner_shipping_id = fields.Many2one("res.partner", "Delivery Address",
        readonly=True, states={"draft": [("readonly", False)]},
        help="Delivery address for current reservation. ", )
    checkin = fields.Datetime("Expected-Date-Arrival", required=True, readonly=True,
        states={"draft": [("readonly", False)]}, )
    checkout = fields.Datetime("Expected-Date-Departure", required=True, readonly=True,
        states={"draft": [("readonly", False)]}, )
    ...
    reservation_line = fields.One2many("hotel_reservation.line", "line_id",
        "Reservation Line", help="Hotel room reservation details.", readonly=True,
        states={"draft": [("readonly", False)]}, )
    state = fields.Selection(
        [("draft", "Draft"), ("confirm", "Confirm"), ("cancel", "Cancel"),
            ("done", "Done")], "State", readonly=True, default=lambda *a: "draft", )
                    ...
                    if room_id.room_reservation_line_ids:
                        for reserv in room_id.room_reservation_line_ids.search(
                            ...
                            if check_in <= reserv_checkout <= check_out:
                                room_bool = True
                            if (
                                    reserv_checkin <= check_in and reserv_checkout >= check_out):
                                room_bool = True
                            mytime = "%Y-%m-%d"
                            ...
                            vals = {"room_id": room_id.id,
                                "check_in": reservation.checkin,
                                "check_out": reservation.checkout, "state": "assigned",
                                "reservation_id": reservation.id, }
                            room_id.write({"isroom": False, "status": "occupied"})
                    else:
                        self.state = "confirm"
                        vals = {"room_id": room_id.id, "check_in": reservation.checkin,
                            "check_out": reservation.checkout, "state": "assigned",
                            "reservation_id": reservation.id, }
                        room_id.write({"isroom": False, "status": "occupied"})
    ...
    def reservation_reminder_24hrs(self):
        ...
        for reserv_rec in self.search([]):
            ...
            if (
                    difference.days == -1 and reserv_rec.partner_id.email and reserv_rec.state == "confirm"):
    ...
    def create_folio(self):
        ...
        for reservation in self:
            ...
            folio_vals = {"date_order": reservation.date_order,
                ...
                "checkin_date": reservation.checkin,
                "checkout_date": reservation.checkout, "duration": duration,
                "reservation_id": reservation.id,
...
class HotelReservationLine(models.Model):
    ...
    reserve = fields.Many2many("hotel.room", "hotel_reservation_line_room_rel",
        "hotel_reservation_line_id", "room_id", domain="[('isroom','=',True),\
                               ('categ_id','=',categ_id)]", )
    ...
    def on_change_categ(self):
        ...
        for room in hotel_room_ids:
            ...
            for line in room.room_reservation_line_ids:
                if line.status != "cancel":
                    if (
                            self.line_id.checkin <= line.check_in <= self.line_id.checkout) or (
            ...
            for rm_line in room.room_line_ids:
                if rm_line.status != "cancel":
                    if (
                            self.line_id.checkin <= rm_line.check_in <= self.line_id.checkout) or (
                            self.line_id.checkin <= rm_line.check_out <= self.line_id.checkout):
                        assigned = True
                    elif (
                            rm_line.check_in <= self.line_id.checkin <= rm_line.check_out) or (
...
class HotelRoomReservationLine(models.Model):
    ...
    state = fields.Selection([("assigned", "Assigned"), ("unassigned", "Unassigned")],
        "Room Status", )
...
class HotelRoom(models.Model):
    ...
    room_reservation_line_ids = fields.One2many("hotel.room.reservation.line",
        "room_id", string="Room Reserve Line")
            ...
            for reserv_line in room.room_reservation_line_ids:
                if reserv_line.status == "confirm":
                    raise ValidationError(_("User is not able to delete the \
                                            ...
                                            in reservation") % (reserv_line.status))
    ...
    def cron_room_line(self):
        ...
        for room in self.search([]):
            ...
            rom_args = [("id", "in", rooms_ids), ("check_in", "<=", curr_date),
                ("check_out", ">=", curr_date)]
...
class RoomReservationSummary(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not self.date_from and self.date_to:
            ...
            last_day = datetime.datetime(last_temp_day.year, last_temp_day.month,
                ...
                if not room.room_reservation_line_ids and not room.room_line_ids:
                    ...
                    for chk_date in date_range_list:
                        ...
                        reservline_ids = reservation_line_obj.search(
                            [("id", "in", reserline_ids), ("check_in", "<=", chk_date),
                                ("check_out", ">=", chk_date),
                                ("state", "=", "assigned")])
                        if not reservline_ids:
                            sdt = dt
                            chk_date = datetime.strptime(chk_date, sdt)
                            chk_date = datetime.strftime(chk_date - timedelta(days=1),
                                sdt)
                            reservline_ids = reservation_line_obj.search(
                                [("id", "in", reserline_ids),
                                    ("check_in", "<=", chk_date),
                        ...
                        if reservline_ids or folio_resrv_ids:
                            room_list_stats.append(
                                {"state": "Reserved", "date": chk_date,
                                    "room_id": room.id, "is_draft": "No",
                                    "data_model": "", "data_id": 0})
                        else:
                            room_list_stats.append(
                                {"state": "Free", "date": chk_date, "room_id": room.id})
...
class QuickRoomReservation(models.TransientModel):
    ...
    def room_reserve(self):
        ...
        for res in self:
            rec = hotel_res_obj.create({"partner_id": res.partner_id.id,
